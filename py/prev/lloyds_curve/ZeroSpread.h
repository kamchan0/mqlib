#ifndef __LIBRARY_PRICERS_FLEXYCF_ZEROSPREAD_H_INCLUDED
#define __LIBRARY_PRICERS_FLEXYCF_ZEROSPREAD_H_INCLUDED
#pragma once
...
namespace FlexYCF
{
    ...
    class ZeroSpread : public CalibrationInstrument
    {
    public:
        ZeroSpread(const std::string& description, const LT::date endDate, const double rate, const LT::date valueDate, GlobalComponentCache& globalComponentCache);
		
		ZeroSpread(const std::string& tenorDescription, const LT::date endDate, const double rate, const LT::date valueDate);
    ...
    private:
		...
		DiscountFactorPtr						m_baseDiscountFactor;
		mutable BaseModelConstPtr               m_dependentModel;
    };

    DECLARE_SMART_PTRS( ZeroSpread )
}
#endif