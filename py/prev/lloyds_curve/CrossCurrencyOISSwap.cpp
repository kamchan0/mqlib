...
namespace
{
	...
	FlexYCF::EndDateCalculationType::Enum_t extractApplicableBackStubEndDateCalculationType(const T& curveParametersTable)
	{
		...
		IDeA::permissive_extract<std::string>(curveParametersTable,
											  ...
											  endDateCalcTypeStr,
											  EndDateCalculationType::toString(CrossCurrencyOISSwap::defaultBackStubEndDataCalculationType()).data());
...
{
	template<>
	const IDeA::DictionaryKey& getKey<CrossCurrencyOISSwap>()
	{
		return IDeA_KEY(YC_INSTRUMENTLIST, CCYOISBASIS);
	}

    using namespace LTQuant;

    CrossCurrencyOISSwap::CrossCurrencyOISSwap(const string& description,
                                         const LT::date fixingDate,
                                         const LT::date startDate,
                                         const LT::date endDate,
                                         const double spread,
		...
		CalibrationInstrument(spread, getKeyName<CrossCurrencyOISSwap>(), description, fixingDate, startDate, endDate),
		...
		m_fixedLeg(FixedLeg::create(FixedLegArguments(valueDate,
													  ...
													  tradeDetails.m_floatAccrualCalendar.string(),tradeDetails.m_currency,tradeDetails.m_index))),
	...
	CalibrationInstrumentPtr CrossCurrencyOISSwap::create(const LTQuant::GenericData& instrumentParametersTable, const LT::Ptr<LT::date>& buildDate, const LTQuant::GenericData& curveParametersTable, const LTQuant::GenericDataPtr&)
	{
        ...
        swapDetails.m_depRateMktConvention.m_depositRateType = DepositRateType::ON;
		...
		return CalibrationInstrumentPtr(new CrossCurrencyOISSwap(maturity, fixingDate, startDate, endDate, 0.0, swapDetails, actualBuildDate, applicableBackStubEndDateCalculationType));
	}

    void CrossCurrencyOISSwap::createInstruments(CalibrationInstruments& instruments, 
                                              ...
                                              const LTQuant::PriceSupplierPtr)
    {
        ...
        for(size_t cnt(0); cnt < nbCcySwaps; ++cnt)
        {
			...
			const string description(IDeA::extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, TENOR), cnt));

			if(!description.empty()) // skip empty rows
            {
				// get rate
                const double rate(IDeA::extract<double>(*instrumentTable, IDeA_KEY(CCYOISBASIS, SPREAD), cnt));
                swapDetails.m_depRateMktConvention.m_rateTenor = LTQC::Tenor(0,1,0,0,0);
				swapDetails.m_depRateMktConvention.m_depositRateType = DepositRateType::ON;
				
				std::string forCcy, forIndex, forLegType, forFrequency, index;
				IDeA::permissive_extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, LEG2CURRENCY), cnt, forCcy, swapDetails.m_forCurrency.string());
				IDeA::permissive_extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, LEG2INDEX), cnt, forIndex, swapDetails.m_forIndex.string());
                IDeA::permissive_extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, LEG2TYPE), cnt, forLegType, swapDetails.m_forLegType.asString().data());
				IDeA::permissive_extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, LEG2FREQUENCY), cnt, forFrequency, swapDetails.m_forFloatFrequency.asString().data());
                IDeA::permissive_extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, LEG1INDEX), cnt, index, swapDetails.m_index.string());
				...
				// build instrument
                CalibrationInstrumentPtr instrument( new CrossCurrencyOISSwap( description,
                                                                            fixingDate,
                                                                            startDate,
                                                                            endDate,
                                                                            rate,
    ...
    void CrossCurrencyOISSwap::updateInstruments(CalibrationInstruments& instrumentList, 
                                              LTQuant::GenericDataPtr instrumentTable, 
                                              size_t* instrumentIndex)
    {
        ...
        {
			//	Description/Tenor (e.g. 5Y)
			const string description(IDeA::extract<std::string>(*instrumentTable, IDeA_KEY(CCYOISBASIS, TENOR), i));
            if(!description.empty())
            {
                const double rate(IDeA::extract<double>(*instrumentTable, IDeA_KEY(CCYOISBASIS, SPREAD), i));
    ...
    const double CrossCurrencyOISSwap::computeModelPrice(const BaseModelPtr model) const
    {
    ...
    void CrossCurrencyOISSwap::accumulateGradient(BaseModel const& baseModel,
                                               double multiplier,
	...
	void CrossCurrencyOISSwap::accumulateGradientConstantDiscountFactor(BaseModel const& baseModel, BaseModel const& dfModel, 
                                        double multiplier,
                                        GradientIterator gradientBegin, 
                                        GradientIterator gradientEnd,
										bool spread)
	...
	void CrossCurrencyOISSwap::accumulateGradientConstantTenorDiscountFactor(BaseModel const& baseModel, BaseModel const& dfModel, 
                                        double multiplier,
                                        GradientIterator gradientBegin, 
                                        GradientIterator gradientEnd,
										bool spread)
    ...
    void CrossCurrencyOISSwap::update()
    {
	...
	double CrossCurrencyOISSwap::getVariableInitialGuess(const double flowTime,
	...
	double CrossCurrencyOISSwap::calculateRateDerivative(const BaseModelPtr& model) const
	{
		return m_fixedLeg->getValue(*model);
	}

	void CrossCurrencyOISSwap::accumulateRateDerivativeGradient(const BaseModel& model,
													  double multiplier,
													  GradientIterator gradientBegin,
	...
	double CrossCurrencyOISSwap::computeParRate(const BaseModelPtr& model)
	{
        double forPV = 0.0;
        if( m_foreignLeg )
        {
	...
	void CrossCurrencyOISSwap::fillRepFlows(IDeA::AssetDomainConstPtr assetDomain,
	...
	double CrossCurrencyOISSwap::getDifferenceWithNewRate(const LTQuant::GenericData& instrumentListData) const
	{
		return doGetDifferenceWithNewRate(instrumentListData, getKey<CrossCurrencyOISSwap>(), IDeA_KEY(CCYOISBASIS, TENOR), IDeA_KEY(CCYOISBASIS, SPREAD));
	}

    ostream& CrossCurrencyOISSwap::print(ostream& out) const
    {
        out << "XOIS" << getDescription().string();
        return out;
    }

   
    ICloneLookupPtr CrossCurrencyOISSwap::cloneWithLookup(CloneLookup& lookup) const
    {
        return ICloneLookupPtr(new CrossCurrencyOISSwap(*this, lookup));
    }

    CrossCurrencyOISSwap::CrossCurrencyOISSwap(CrossCurrencyOISSwap const& original, CloneLookup& lookup) : 
    ...
    void CrossCurrencyOISSwap::fillCashFlowsTable(LTQuant::GenericData& cashFlowsTable) const
	{
	...
	void CrossCurrencyOISSwap::fillCashFlowPVsTable(const BaseModel& model,
												 LTQuant::GenericData& cashFlowPVsTable) const
	{
     ...
     void CrossCurrencyOISSwap::finishCalibration(const BaseModelPtr model)
     {
     ...
     void CrossCurrencyOISSwap::reloadInternalState(const FlexYCF::CalibrationInstrumentPtr& src)
     {
         const CrossCurrencyOISSwapPtr ourTypeSrc=std::tr1::static_pointer_cast<CrossCurrencyOISSwap>(src);
         if(!ourTypeSrc)
         {
         ...
         if(m_foreignLeg)
         {
            m_firstForeignDiscountFactor = ourTypeSrc->m_firstForeignDiscountFactor;
            m_lastForeignDiscountFactor = ourTypeSrc->m_lastForeignDiscountFactor;
         }
         CalibrationInstrument::reloadInternalState(src);
     }

     void CrossCurrencyOISSwap::initializeForeignLegPricing(const BaseModelPtr model) const
        {
           initializeForeignLegPricing(*(model.get()));
        }
         
        void CrossCurrencyOISSwap::initializeForeignLegPricing(const BaseModel& model) const
        {
            if( !m_isInitialized )
            {
                LT::Str market; 
