...
*****************************************************************************/
#ifndef __LIBRARY_PRICERS_FLEXYCF_FLATLEFTINTERPOLATION_H_INCLUDED
#define __LIBRARY_PRICERS_FLEXYCF_FLATLEFTINTERPOLATION_H_INCLUDED
#pragma once
...
namespace FlexYCF
{
    /// A class to perform flat left interpolation:
    /// if the knots are given by {(x[i], y[i]); 0 <= i <= n}
    /// then interpolation is f(x) = y[i] if x is such that
    /// x[i-1] < x <= x[i]
    class FlatLeftInterpolation: public InterpolationMethod
    {
    public:
     ...
     DECLARE_SMART_PTRS(FlatLeftInterpolation)

}   // FlexYCF
#endif //__LIBRARY_PRICERS_FLEXYCF_FLATLEFTINTERPOLATION_H_INCLUDED